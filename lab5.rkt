...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname lab5) (read-case-sensitive #t) (teachpacks ((lib "image.rkt" "teachpack" "2htdp") (lib "batch-io.rkt" "teachpack" "2htdp") (lib "universe.rkt" "teachpack" "2htdp"))) (htdp-settings #(#t write repeating-decimal #f #t none #f ((lib "image.rkt" "teachpack" "2htdp") (lib "batch-io.rkt" "teachpack" "2htdp") (lib "universe.rkt" "teachpack" "2htdp")) #f)))
...
;; dado um predio, cria uma imagem do tal, com contorno apropriado
...
;; constantes:
(define METEORO1 (make-meteoro 20 200 10 'd 4 1 0))
(define METEORO2 (make-meteoro 450 0 15 'e 5 2 0))
(define METEORO3 (make-meteoro 350 0 5 'e 6 3 0))
...
(define/contract (desenha-meteoro met)
  (-> meteoro? image?)
...
(define/contract (escolhe-cor-meteoro pr)
  (-> meteoro? string?)
  (cond
...
(define/contract (desenha-meteoro-caindo met c)
  (-> meteoro? image? (listof image?))
  (cond
...
(define/contract (meteoro-caiu? met c)
  (-> meteoro? image? boolean?)
  (let
...
(define/contract (avanca-meteoro met)
  (-> meteoro? meteoro?)
  (cond
...
(define/contract (desenha-contorno pred)
  (-> predio? image?)
  (let ([cor
         (cond
...
(define/contract (meteoro-impacto met pred)
  (-> meteoro? predio? boolean?)
...
(define/contract (chuva-de-meteoros met lista-p cena)
  (-> meteoro? (listof predio?) image? (listof image?))
...
(define/contract (atualiza-predios met lista-p)
  (-> meteoro? (listof predio?) (listof predio?))
  (local
...
(define/contract (inicializa-meteoro m)
  (-> meteoro? meteoro?)
...
(define/contract (grande-chuva-de-meteoros meteoros predios cena)
  (-> (listof meteoro?) (listof predio?) image? (listof image?))
  (local
...
(define/contract (atualiza-predios-lista lm lp)
  (-> (listof meteoro?) (listof predio?) (listof predio?))
  (cond
    [(empty? lm) lp]
    [else
...
(define/contract (meteoro-acabou? m lista-p cena)
  (-> meteoro? (listof predio?) image? boolean?)
  (or
